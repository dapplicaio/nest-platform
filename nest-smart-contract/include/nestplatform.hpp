...
CONTRACT nestplatform : public eosio::contract {
  public:
    ...
    ////////////////////////////////////////////////////////////////////////////////
    ACTION createlboard(eosio::name owner, string boardname, uint64_t gameid, double startpoints);
    ...
    ACTION changebpoint(uint64_t boardid, double startpoints);
    ...
    ACTION gamerate(eosio::name username, uint64_t gameid, double rate, std::string comment);
    ACTION apprcomment(uint64_t id, bool accept);
    ...
    ACTION userregister(eosio::name username,std::string avatar, std::string nickname, eosio::name referrer);
    ...
    ACTION addreferral(eosio::name referrer, eosio::name referee);
    ...
    //        CARDS ACTIONS
    ////////////////////////////////////////////////////////////////////////////////
    ACTION cardcreate(eosio::name owner, uint64_t gameid, std::string cardname, std::string enableimg, std::string disableimg, std::string series, eosio::name schema_name);
    ACTION carddelete( uint64_t cardid);
    ACTION carddrop(uint64_t cardid, eosio::name username, uint8_t nftstandard);
    ACTION cardapprove(uint64_t cardid, bool approve, int32_t template_id);
  ...
  private:
    ...
    TABLE rategame
    {
      ...
      double                    totalstars{0.0};    //total stars on game
      uint64_t                  totalvotes{0};      //total votes on game
      double                    rate{0.0};          //game's rate
      std::vector<rate_record>  records;             //user record
    ...
    typedef eosio::multi_index<eosio::name("comentapprs"), comentaprove> comentapprs;
    ...
    {
      eosio::name               account;            //user's WAX account
      eosio::name               referrer;           //user's referrer
      std::vector<eosio::name>  referee;            //user's referees

      uint64_t primary_key() const {return account.value;}
    };

    typedef eosio::multi_index<eosio::name("referrals"), referral> referrals;
    ...
    typedef eosio::multi_index<eosio::name("dcards"), dcard> cards;
    ...
    typedef eosio::multi_index<eosio::name("apprdcards"), apprdcard> apprcards;
    ...
    TABLE startlpoint
    {
      ...
      uint64_t primary_key() const {return boardid;}
    };

    typedef eosio::multi_index<eosio::name("startlpoints"), startlpoint> startlpoints;
